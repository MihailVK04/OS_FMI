...
int main(int argc, char* argv[]) {
    if(argc!=4) {
        errx(1,"Incorrect parameters count");
    }
    int fd1 = open(argv[1],O_RDONLY);
    if (fd1<0) {
        err(5,"Could not open the first file");
    }
    int fd2 = open(argv[2],O_RDONLY);
    if (fd2<0) {
        err(6,"Could not open the second file");
    }
    if(getFileSize(fd1) != getFileSize(fd2)) {
        errx(7,"Files differ in size");
    }
    int fd3 = open(argv[3],O_WRONLY | O_CREAT | O_TRUNC,0666);
    if (fd3<0) {
        err(8,"Error opening the third file");
    }
    Triple t;
